...
public final class TableDAO_Impl implements TableDAO {
  ...
  @Override
  public TableEntity getById(final int id) {
    final String _sql = "SELECT * FROM Cafe_Table WHERE Table_Id = ? LIMIT 1";
    return DBUtil.performBlocking(__db, true, false, (_connection) -> {
      final SQLiteStatement _stmt = _connection.prepare(_sql);
      try {
        int _argIndex = 1;
        ...
        if (_stmt.step()) {
          _result = new TableEntity();
          _result.Table_Id = (int) (_stmt.getLong(_columnIndexOfTableId));
          if (_stmt.isNull(_columnIndexOfTableName)) {
            _result.Table_Name = null;
          } else {
            _result.Table_Name = _stmt.getText(_columnIndexOfTableName);
          }
